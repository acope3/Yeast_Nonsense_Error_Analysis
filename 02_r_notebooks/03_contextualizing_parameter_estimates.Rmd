...
main.yeast.result.path <- "../01_results/00_panse_fits/2026-01-07_Weinberg_etal_2016_all_frames_filter_genes_shorter_than_225_codons_200_ramp_updated_posterior_variable_nse/final_restart/"



# main.yeast.data.path <- "../00_data/00_panse_input/2025-11-21_ferguson_etal_2023_all_frames_100_ramp.csv"
# full.yeast.data.path <- "../00_data/00_panse_input/00_unfiltered_genes/ferguson_etal_2023_ottr_p1_rep1_plus_rep2_all_genes.csv"
# main.yeast.result.path <- "../01_results/00_panse_fits/2025-11-21_Ferguson_etal_2023_rep1_plus_rep2_all_frames_filter_genes_shorter_than_150_codons_100_ramp/final_restart/"

main.yeast.data <- read_csv(main.yeast.data.path)
full.yeast.data <- read_csv(full.yeast.data.path)
if ("gene" %in% colnames(full.yeast.data))
{
  full.yeast.data <- full.yeast.data %>%
    dplyr::rename(GeneID=gene) %>%
    filter(!str_detect(GeneID,"Q0[0-9]{3}"))
}
...
mrna.vs.panse<- ggplot(sigma,aes(x=RPKM,y=Phi)) +
  geom_point(show.legend = F) +
  stat_cor(method="spearman",label.sep="\n",cor.coef.name = "rho",label.y=1.25) +
...
panse.wait.times.vs.elong.rates <- ggplot(ribo.wt.w.panse,aes(x=mean_elongation_rate,y=Mean,label=Codon)) +
  ...
  stat_cor(method="spearman",label.sep="\n",label.x.npc = "center",cor.coef.name = "rho") +
...
panse.vs.trna.wt <- ggplot(ribo.wt.w.panse,aes(x=`(tRNA_GCN*Wobble)`,y=1/Mean)) +
  geom_point() +
  stat_cor(method="spearman",label.sep="\n",cor.coef.name = "rho",label.x.npc = "center",label.y.npc = "bottom") +
  xlab("Elongation Rates"~c[c]~"(tGCN * wobble)") +
  ylab(expression("Elongation Rates"~c[c]~"(PANSE)")) +
...
fig.1
ggsave3(file.path(fig.dir, "Figure_1.pdf"),plot=fig.1,width=8.7,height = 8.7)
...
sigma.effects
ggsave3(file.path(fig.dir, "Figure_4.pdf"),plot=sigma.effects,width=14,height=5)
...
```{r}
same <- getParameterDataFrames("../01_results/00_panse_fits/2026-01-07_Weinberg_etal_2016_all_frames_filter_genes_shorter_than_225_codons_200_ramp_updated_posterior_shared_nse/final_restart/")
...
sorted.nse.rate.df <- nse.rate.df %>%
  arrange(AA,Mean) %>%
  mutate(Codon = ifelse(!Codon %in% c("CTT","GGG","CCC","AGG","CTC","CCT","CTG","CGA","CCG","GTG","GCG"),Codon,paste0("**",Codon,"**")),
...
nse.rate.by.codon <- ggplot(sorted.nse.rate.df,aes(x=Codon,y=Mean)) +
  ...
  theme(axis.text.x = ggtext::element_markdown(angle = 90, vjust = 0.5, hjust=1,size=8),
...
sorted.nse.pr.df <- nse.pr.df %>%
  ...
  mutate(Codon = ifelse(!Codon %in% c("CTT","GGG","CCC","AGG","CTC","CCT","CTG","CGA","CCG","GTG","GCG"),Codon,paste0("**",Codon,"**")),
         Codon = factor(Codon,levels = unique(Codon)),
         By.wobble = ifelse(tRNA == 0,"Wobble Only","Exact Match")) 
...
nse.pr.by.codon <- ggplot(sorted.nse.pr.df %>%
  ...
  scale_y_continuous(labels = function(x) label_parsed(paste("10^",x))) +
  theme(axis.text.x = ggtext::element_markdown(angle = 90, vjust = 0.5, hjust=1,size=12),
        aspect.ratio = 0.75,
...
```{r}
err.detect <- read_csv("../00_data/01_correlates/landerer_etal_error_detection.csv")

log.nserate <- main.yeast.result$Log.NSERate %>%
  left_join(err.detect) %>%
  mutate(Frameshift.competent = ifelse(!Codon %in% c("CTT","GGG","CCC","AGG","CTC","CCT","CTG","CGA","CCG","GTG","GCG"),"No","Yes")
)

nserate.vs.mse.prob <- ggplot(log.nserate,aes(x=mean,y=Mean)) +
  geom_point(aes(color=Frameshift.competent)) +
  scale_color_manual(values=c("Yes"="red","No"="black")) +
  geom_errorbar(aes(ymin=`2.5%`,ymax=`97.5%`),alpha=0.5) +
  geom_errorbarh(aes(xmin=mean-se,xmax=mean+se),alpha=0.5) +
  labs(color="Frameshift\ncompetent") +
  ylab(expression("Log"[10]*"(NSE Rate)")) +
  xlab(expression("Log"[10]*"(Missense Error Probability)")) +
  theme_cowplot() +
  geom_abline(intercept = 0,slope=1,linetype="dashed") +
  scale_x_log10() +
  stat_cor(method="spearman",label.sep="\n",cor.coef.name = "rho",label.y = -6,label.x = -3.25,size=4) 
nserate.vs.mse.prob
log.nserate.fs <- log.nserate %>% split(~Frameshift.competent)

nserate.fs <- ggplot(log.nserate,aes(x=Frameshift.competent,y=Mean)) +
  geom_boxplot() +
  ylab(expression("Log"[10]*"(NSE Rate)")) +
  xlab("Frameshift competent sense codon") +
  theme_cowplot() +
  stat_compare_means(method="t.test",vjust = 0.5,label.x = 1.25,size=4)
nserate.fs
t.test(log.nserate.fs$No$Mean,log.nserate.fs$Yes$Mean)
...
```{r}
trna.comp <- read_tsv("../01_results/tgcn.tsv")
scer.tgcn <- read_tsv("../00_data/01_correlates/tgcn.tsv")

wobble <- trna.comp %>% 
  dplyr::select(Codon,AA,Cognate) %>% 
  separate(Cognate,sep=",",into=c("tRNA.1","tRNA.2","tRNA.3")) %>% 
  rowwise() %>% 
  mutate(across(starts_with("tRNA"),~ifelse(!is.na(.),reverseComplement(.),NA))) %>% 
  ungroup() %>% 
  mutate(Total.tRNA=rowSums(!is.na(dplyr::select(.,-Codon,-AA))), 
         Perfect.Match = ifelse(Codon %in% c(tRNA.1,tRNA.2),T,F),
         Wobble = case_when(
           Perfect.Match & Total.tRNA == 1 ~ "No",
           Perfect.Match & Total.tRNA > 1 ~ "Yes",
           !Perfect.Match ~ "Yes"
         ))

err.detect <- read_csv("../00_data/01_correlates/landerer_etal_error_detection.csv") %>%
  mutate(Log10.Miss.Err.Prob = log10(mean))
...
nse.df.lst <- lapply(nse.df.tmp, function(x) {
  ...
  left_join(err.detect) %>%
  left_join(wt.next) %>%
  left_join(main.yeast.result$Wait.Time %>% dplyr::select(Codon,Mean) %>% dplyr::rename(Wait.Time=Mean)) %>%
  separate(Codon,into=c("First","Second","Third"),sep="(?<=[A-Z])",remove = F) %>%
  mutate(Stop.Neighbor.First.Yes = ifelse(Stop.Neighbor.First == 0,"No","Yes"),
         ...
         Frameshift.competent = ifelse(!Codon %in% c("CTT","GGG","CCC","AGG","CTC","CCT","CTG","CGA","CCG","GTG","GCG"),"No","Yes")
)
})

names(nse.df.lst)

## The full model has very large VIFs (>5), so it is hard to trust these results
## This appears to be due to the inclusion of the nucleotide at each codon position
full.model.fit.lst <- lapply(nse.df.lst,
                         function(x) {
                           df <- as_data_frame(x)
                           lm(Mean ~ First + Second + Third + Stop.Neighbor.First + Stop.Neighbor.Second + Stop.Neighbor.Third + Log10.Miss.Err.Prob + Frameshift.competent,data=df,weights = 1/(Std.Dev^2))})


## Solution: separate the regressions
model.fit.lst <- lapply(nse.df.lst,
                         function(x) {
                           df <- as_data_frame(x)
                           lm(Mean ~ Stop.Neighbor.First + Stop.Neighbor.Second + Stop.Neighbor.Third + Log10.Miss.Err.Prob + Frameshift.competent,data=df,weights = 1/(Std.Dev^2))})

model.nt.pos.lst <- lapply(nse.df.lst,
                         function(x) {
                           df <- as_data_frame(x)
                           lm(Mean ~ First + Second + Third,data=df,weights = 1/(Std.Dev^2))})




df <- as_data_frame(nse.df.lst[["Rate"]])
alex.w.weights.best.fits <-
 MASS::stepAIC(model.fit.lst[["Rate"]],
               direction = "both",
               trace = FALSE,
               criteria="AIC")
...
coef_data <- coef_data %>%
  mutate(term=case_when(
    ...
    term == "Frameshift.competentYes" ~ "Frameshift competent",
    term == "WobbleYes" ~ "Wobble base pairing",
    term == "Log10.Miss.Err.Prob" ~ "Missense error probability"
  ))
...
plot_data <- data.frame(
  ...
  mutate(term=fct_relevel(term,"Intercept (Reference class)",after = Inf))
...
p <- (nse.rate.by.codon | (free(nserate.vs.mse.prob / nserate.fs) / reg.plot)) + 
  plot_annotation(tag_levels = "A") + 
  plot_layout(widths=c(2,1))
p


ggsave3(file.path(fig.dir, "Figure_2.pdf"),p,width=10.5,height=12)
...
wt.vs.pos <- compareParameterVsPosition(full.yeast.data,
                                         main.yeast.result,
                                         parameter.name="Wait.Time",
                                         xlab="Waiting time",
                                         title="Waiting time vs. Average Position",
                                         include.stop.codon.neighbor = F,
                                         flip.coord=T,
                                         correlation.label.pos = c(-3,0.53))
...
plotCodonEnrichment <- function(rfp.data,use.normalized.position=T,cutoff.length=0.25,min.length=0)
{
  gene.length <- rfp.data %>%
    count(GeneID) %>%
    filter(n > min.length) %>%
  ...
  termini <- cutoff.length
  
  rfp.data<- rfp.data %>% 
    left_join(gene.length) %>%
    group_by(GeneID) %>% 
  ...
  if(use.normalized.position)
  {
    end5 <- rfp.data %>%
      filter(Normalized.Position < termini)
   
     end3 <- rfp.data %>%
      filter(Normalized.Position > (1 - termini))
     
    mid <- rfp.data %>%
      filter(Normalized.Position >= termini & Normalized.Position <= (1 - termini))
  } else
  {
    end5 <- rfp.data %>%
      filter(Position < termini)
   
     end3 <- rfp.data %>%
      filter(Position > (n - termini))
     
     mid <- rfp.data %>%
       filter(Position >= termini & Position <= (n - termini))
  }
  
  exp.freq <- mid %>%
    group_by(AA,Codon) %>%
    summarize(Exp.Count= n()) %>%
    ungroup() %>%
    group_by(AA) %>%
    mutate(Exp.Freq = Exp.Count/sum(Exp.Count))
  ...
  end5.count <- end5.count %>% 
    left_join(exp.freq)
  end3.count <- end3.count %>% 
    left_join(exp.freq)
  
  end5.test <- end5.count %>% 
    group_by(AA,Codon) %>%
    nest() %>%
    mutate(binom = purrr::map(data,~binom.test(.x$Obs.Count,.x$Total.AA,.x$Exp.Freq,alternative="greater") %>% broom::tidy())) %>%
    dplyr::select(-data) %>%
    unnest(c(binom)) %>%
    left_join(main.yeast.result$Log.NSEProb,by=c("AA","Codon")) %>%
    ungroup() %>%
    mutate(adj.p.value=p.adjust(p.value,method="BH"),
           Significant= ifelse(adj.p.value < 0.05,"Enriched","No Difference"))
  
  end3.test <- end3.count %>% 
    group_by(AA,Codon) %>%
    nest() %>%
    mutate(binom = purrr::map(data,~binom.test(.x$Obs.Count,.x$Total.AA,.x$Exp.Freq,alternative="greater") %>% broom::tidy())) %>%
    dplyr::select(-data) %>%
  ...
  enrich.5.end <- ggplot(end5.test,aes(x=Significant,y=Mean)) + 
    geom_boxplot() + 
    xlab("5'-end") +
    ylab(expression("Log"[10]*"(Pr(NSE))")) + 
    stat_compare_means()
  enrich.3.end <- ggplot(end3.test,aes(x=Significant,y=Mean)) + 
    geom_boxplot() + 
    xlab("3'-end") +
    stat_compare_means() +   
    ylab(expression("Log"[10]*"(Pr(NSE))"))
  ...
  return(list(enrich.5.plot = enrich.5.end,enrich.3.plot=enrich.3.end))
}

rel.termini <- plotCodonEnrichment(rfp.data = full.yeast.data,use.normalized.position = T,cutoff.length = 0.25)
abs.termini <- plotCodonEnrichment(rfp.data = full.yeast.data,use.normalized.position = F,cutoff.length = 100,min.length = 250)
...
total <- (rel.termini[[1]] | rel.termini[[2]] | prob.by.pos) + plot_annotation(tag_levels = c("A","","B")) + plot_layout(widths = c(1,1, 2))
total

abs.termini.plot <- (abs.termini[[1]] | abs.termini[[2]]) + plot_annotation(tag_levels = c("A","","B"))
abs.termini.plot
ggsave3(file.path(fig.dir, "Figure_5.pdf"),total,width = 20,height=6)

ggsave3(file.path(fig.dir, "codon_enrichment_abs_cutoff.pdf"),abs.termini.plot,width = 10,height=5)
...
mrna.vs.sigma <- ggplot(sigma,aes(x=RPKM,y=Sigma)) +
  geom_point(aes(color=log10(Length))) +
  stat_cor(method="spearman",label.sep="\n",label.y.npc = "bottom",cor.coef.name = "rho") +
...
mrna.vs.sigma.avg <- ggplot(sigma,aes(x=RPKM,y=Sigma^(1/Length))) +
  geom_point(aes(color=log10(Length))) +
  stat_cor(method="spearman",label.sep="\n",label.x.npc = "center",label.y.npc = "bottom",cor.coef.name = "rho") +
  xlab("mRNA Abundance (RPKM)") +
  scale_x_log10() +
...
sel.vs.nse <- ggplot(deta,aes(y=Rel.Log.NSEProb,x=DEta)) +
  geom_point() +
  stat_cor(method="spearman",label.sep="\n",label.x = 1,label.y.npc = "bottom",cor.coef.name = "rho") +
...
prob.by.pos.by.expr <- ggplot(sigma.per.pos.per.bin,aes(x=Position,y=Sigma.i,color=bin)) +
  ...
  stat_regline_equation(formula =y ~ log(x),label.x = 10,show.legend = F,aes(label = sub("x", "log(x)", ..eq.label..))) +
  stat_cor(label.x.npc = "center",method="spearman",show.legend = F,cor.coef.name = "rho") +
  ylab("Mean Pr(No NSE)") +
  scale_color_viridis_d() +
...
# Correlations between codon-specific parameters

```{r}
alpha.df <- main.yeast.result$Alpha %>% 
  dplyr::rename(Alpha=Mean,Alpha_Std.Dev=Std.Dev,`Alpha_2.5%`=`2.5%`,`Alpha_97.5%`=`97.5%`)
lambda.df <- main.yeast.result$Lambda %>% 
  dplyr::rename(Lambda=Mean,Lambda_Std.Dev=Std.Dev,`Lambda_2.5%`=`2.5%`,`Lambda_97.5%`=`97.5%`)
wt.df <- main.yeast.result$Wait.Time %>% 
  dplyr::rename(Wait.Time=Mean,Wait.Time_Std.Dev=Std.Dev,`Wait.Time_2.5%`=`2.5%`,`Wait.Time_97.5%`=`97.5%`)
log.nserate.df <- main.yeast.result$Log.NSERate %>% 
  dplyr::rename(Log.NSERate=Mean,Log.NSERate_Std.Dev=Std.Dev,`Log.NSERate_2.5%`=`2.5%`,`Log.NSERate_97.5%`=`97.5%`)

csp.param.df <- list(alpha.df,lambda.df,wt.df,log.nserate.df) %>%
  purrr::reduce(left_join,by=c("AA","Codon")) %>%
  rowwise() %>%
  mutate(Stop.neighbors = stopCodonNeighbor(Codon,by.pos=3))

ggplot(csp.param.df,aes(x=Alpha,y=Log.NSERate)) +
  geom_point(aes(color=Stop.neighbors)) +
  geom_errorbar(aes(ymin=`Log.NSERate_2.5%`,ymax=`Log.NSERate_97.5%`),alpha=0.5) +
  geom_errorbarh(aes(xmin=`Alpha_2.5%`,xmax=`Alpha_97.5%`),alpha=0.5) +
...
ggplot(csp.param.df,aes(x=Lambda,y=Log.NSERate)) +
  geom_point(aes(color=Stop.neighbors)) +
  geom_errorbar(aes(ymin=`Log.NSERate_2.5%`,ymax=`Log.NSERate_97.5%`),alpha=0.5) +
  geom_errorbarh(aes(xmin=`Lambda_2.5%`,xmax=`Lambda_97.5%`),alpha=0.5) +
...
ggplot(csp.param.df,aes(x=Wait.Time,y=Log.NSERate)) +
  geom_point(aes(color=Stop.neighbors)) +
  geom_errorbar(aes(ymin=`Log.NSERate_2.5%`,ymax=`Log.NSERate_97.5%`),alpha=0.5) +
  geom_errorbarh(aes(xmin=`Wait.Time_2.5%`,xmax=`Wait.Time_97.5%`),alpha=0.5) +
  xlab(expression(alpha/lambda)) +
  ylab(expression("Log"[10]*"(NSE Rate)")) +
  stat_cor(method="spearman",cor.coef.name = "rho") +
  theme_cowplot() +
  theme(aspect.ratio=1)
...
```{r}
getNonCognate <- function(codon,tgcn)
{
  anticodon.cognate <- scer.tgcn %>%
    filter(Codon == codon) %>%
    dplyr::select(Near.cognate) %>%
    mutate(anticodon=paste(Near.cognate,sep=",")) %>%
    dplyr::select(anticodon) %>% 
    deframe() %>% 
    str_split(",") %>% 
    unlist()
  
  anticodon.cognate <- anticodon.cognate[which(anticodon.cognate != "NA")]
  cognate <- lapply(anticodon.cognate,reverseComplement) %>% unlist()
  non.cogn.tgcn <- tgcn %>%
    filter(Codon %in% cognate) %>%
    summarize(sum(tRNA)) %>%
    deframe() %>%
    unname()
  return(non.cogn.tgcn)
  
}

scer.tgcn <- read_tsv("../00_data/01_correlates/tgcn.tsv")
trna.cog <- read_tsv("../01_results/tgcn.tsv")
scer.tgcn <- scer.tgcn %>%
  left_join(trna.cog)
nserate <- main.yeast.result$Log.NSERate
wait.time <- main.yeast.result$Wait.Time

sense.codons <- AnaCoDa::codons()
sense.codons <- sense.codons[!sense.codons %in% c("TGA","TAA","TAG")]
names(sense.codons) <- sense.codons

noncognate.tgcn <- lapply(sense.codons,getNonCognate,scer.tgcn) %>%
  bind_rows(.id="Codon") %>%
  pivot_longer(everything(),names_to="Codon",values_to="Non_cognate_tGCN")

test <- main.yeast.result$Log.NSERate %>%
  left_join(wait.time,by=c("AA","Codon"),suffix=c("_NSE","_WT")) %>%
  left_join(noncognate.tgcn)

plot(test$Mean_NSE,test$Non_cognate_tGCN)
cor.test(test$Mean_NSE,test$Non_cognate_tGCN,method = "spearman")
