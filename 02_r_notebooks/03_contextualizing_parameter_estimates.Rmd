...
Note that most of this code was written using the convention of $\phi$ for our gene-specific rates. However, the manuscript uses $\kappa'$. I'm sticking with $\phi$ in the code for PANSE to avoid breaking things, but keep in mind this is different from the ROC-SEMPPR estimate of $\phi$, which is based solely on codon usage patterns.
...
sigma.for.si <- sigma %>%
  dplyr::rename(PANSE.kappa=Phi,
                Bonnin.Drop.Off.Resilience=drop_off_resilience,
                Weinberg.mRNA.RPKM=RPKM,
                Weinberg.Init.Rate=S,
                ROC.phi=Mean) %>%
  dplyr::select(-c(Number_of_codons,rho_phys,alpha_phys,ref))

write_tsv(sigma.for.si,"../01_results/sigma_for_si.tsv")
...
bonnin.vs.sigma <- ggplot(sigma,aes(x=drop_off_resilience,y=Sigma)) +
  ...
  labs(color=expression("Log"["10"]*"(Gene Expr."~kappa^"'"*")"),size=expression("Log"["10"]*"(Length)")) +
  theme(ggside.axis.text = element_blank(),
        ggside.axis.line = element_blank(),
        ggside.axis.ticks = element_blank()) +
  geom_xsidehistogram(fill="white",color="black") +
...
write_tsv(plot_data,"../01_results/nserate_vs_codon_properties_regression.tsv")
